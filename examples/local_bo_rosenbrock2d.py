...
def run(smoke: bool = False, show_plots: bool = False) -> float:
    ...
    for _ in range(n1):
        bo.step(mode="local", acq_type="UCB", plot_acq=show_plots)
        if show_plots:
            plt.show()
    for _ in range(n2):
        bo.step(mode="local", acq_type="qUCB", plot_acq=show_plots)
        if show_plots:
            plt.show()
    bo.step(mode="finetune", acq_type="qUCB", plot_acq=show_plots)
    bo.finalize()
